		...
		while (true) {
			...
			for (int i = 0; i < total_points; i++) { 
				...
				for (int cluster_id = 0; cluster_id < K; cluster_id++) { // loop through all the clusters (find closest cluster)
					...
					if (clusters[cluster_id].getLastChange() >= iter - 1) { // check the last time it was updated (changed since last iteration)
						double sum = 0.0;
						
						for (int j = 0; j < total_values; j++) {
							double diff = clusters[cluster_id].getCentralValue(j) - p.getValue(j);
							sum += diff * diff;
						}
