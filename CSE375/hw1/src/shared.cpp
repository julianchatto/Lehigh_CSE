...
map<int, unique_ptr<shared_mutex>> account_locks; // Lock for each account
int NUM_ACCOUNTS = 10;
...
void deposit() {
    ...
    // lock the accounts
    unique_lock<shared_mutex> lock1(*account_locks[account1]);
    unique_lock<shared_mutex> lock2(*account_locks[account2]);
