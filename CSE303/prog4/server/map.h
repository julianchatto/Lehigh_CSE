...
public:
	...
	virtual bool insert(K key, V val, function<void()> on_success) = 0;
	...
	virtual bool upsert(K key, V val, function<void()> on_ins, function<void()> on_upd) = 0;
	...
	virtual bool remove(K key, function<void()> on_success) = 0;
	...
	///             useful for 2pl
	virtual void do_all_readonly(function<void(const K, const V &)> f, function<void()> then) = 0;
};
