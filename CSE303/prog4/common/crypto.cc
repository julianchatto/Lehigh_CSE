...
vector<uint8_t> create_aes_key() {
	...
	if (!RAND_bytes(key.data(), AES_KEYSIZE) || !RAND_bytes(key.data() + AES_KEYSIZE, AES_IVSIZE))
...
EVP_CIPHER_CTX *create_aes_context(const vector<uint8_t> &key, bool encrypt) {
	...
	if (ctx == nullptr)
		return err<EVP_CIPHER_CTX *>(nullptr, "Error: OpenSSL couldn't create context: ", ERR_error_string(ERR_get_error(), 0));
	...
	if (!EVP_CipherInit_ex(ctx, EVP_aes_256_cbc(), 0, 0, 0, encrypt))
		return err<EVP_CIPHER_CTX *>(nullptr, "Error: OpenSSL couldn't initialize context: ", ERR_error_string(ERR_get_error(), 0));
	if ((EVP_CIPHER_CTX_key_length(ctx) != AES_KEYSIZE) || (EVP_CIPHER_CTX_iv_length(ctx) != AES_IVSIZE))
		return err<EVP_CIPHER_CTX *>(nullptr, "Error: OpenSSL couldn't initialize context: ", ERR_error_string(ERR_get_error(), 0));

	// Set the key and iv on the AES context, and set the mode to encrypt or
	// decrypt
	if (!EVP_CipherInit_ex(ctx, nullptr, nullptr, key.data(), key.data() + AES_KEYSIZE, encrypt))
		return err<EVP_CIPHER_CTX *>(nullptr, "Error: OpenSSL couldn't re-init context: ", ERR_error_string(ERR_get_error(), 0));
...
bool reset_aes_context(EVP_CIPHER_CTX *ctx, vector<uint8_t> &key, bool encrypt) {
	if (!EVP_CipherInit_ex(ctx, nullptr, nullptr, key.data(), key.data() + AES_KEYSIZE, encrypt)) {
		EVP_CIPHER_CTX_cleanup(ctx);
		return err(false, "Error: OpenSSL couldn't re-init context: ", ERR_error_string(ERR_get_error(), 0));
	}
	return true;
}
...
EVP_PKEY *init_RSA(const string &basename) {
	...
	if (!pub_exists && !pri_exists)
		...
		return err<EVP_PKEY *>(nullptr, "Error: cannot find ", basename.c_str(), ".pri");
	else if (!pub_exists && pri_exists)
		return err<EVP_PKEY *>(nullptr, "Error: cannot find ", basename.c_str(), ".pub");
