...
int reliable_get_to_eof_or_n(int sd, vector<uint8_t>::iterator pos, int amnt) {
	...
	while (remain) {
		...
		if (rcd <= 0) {
			if (errno != EINTR)
				return (rcd == 0) ? total : err(-1, "Error in recv(): ", msg_from_errno(errno).c_str());
		} else {
			next_byte += rcd;
			remain -= rcd;
			total += rcd;
...
vector<uint8_t> reliable_get_to_eof(int sd) {
	...
	while (true) {
		...
		// On error, fail for non-EINTR, no-op on EINTR
			...
			if (errno != EINTR) return err<vector<uint8_t>>( {}, "Error in recv(): ", msg_from_errno(errno).c_str());
		}
...
int connect_to_server(const string &hostname, int port) {
	...
	if (sd < 0)
		return err(-1, "Error making client socket: ", msg_from_errno(errno).c_str());
	if (connect(sd, (sockaddr *)&addr, sizeof(addr)) < 0) {
		close(sd);
		return err(-1, "Error connecting socket to address: ", msg_from_errno(errno).c_str());
	}
	return sd;
}
...
int create_server_socket(size_t port) {
	...
	int sd = socket(AF_INET, SOCK_STREAM, 0);
	if (sd < 0)
		return err(-1, "Error making server socket: ", msg_from_errno(errno).c_str());
	...
	if (setsockopt(sd, SOL_SOCKET, SO_REUSEADDR, &tmp, sizeof(int)) < 0)
		return err(-1, "setsockopt(SO_REUSEADDR) failed: ", msg_from_errno(errno).c_str());
	...
	if (bind(sd, (struct sockaddr *)&addr, sizeof(addr)) < 0)
		return err(-1, "Error binding socket to local address: ", msg_from_errno(errno).c_str());

	if (listen(sd, 1024) < 0)
		return err(-1, "Error listening on socket: ", msg_from_errno(errno).c_str());
...
bool accept_client(int sd, function<bool(int)> handler) {
	...
	while (true) {
		...
		if (connSd < 0) {
			close(sd);
			return err(false, "Error accepting request from client: ", msg_from_errno(errno).c_str());
		}
		ContextManager cs([&]() { close(connSd); });
		char cliName[1024];
		cout << "Connected to " << inet_ntop(AF_INET, &clientAddr.sin_addr, cliName, sizeof(cliName)) << endl;
