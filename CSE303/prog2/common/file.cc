...
vector<uint8_t> load_entire_file(const string &filename) {
	...
	if (recd != res.size() || !feof(f))
		return err<vector<uint8_t>>({}, "Incorrect number of bytes read from ", filename.c_str());
	return res;
}
...
bool write_file(const string &filename, const vector<uint8_t> &data, size_t skip) {
	...
	//     counts and EINTR.
	if (fwrite(data.data() + skip, sizeof(char), data.size() - skip, f) != data.size() - skip)
		return err(false, "Incorrect number of bytes written to ", filename.c_str());
	return true;
}
