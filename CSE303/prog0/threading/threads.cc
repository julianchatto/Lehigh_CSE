 ...
 */
void run_timed_test(arg_t &args, function<void(int)> task) {
	...
	thread threads[args.num_threads];
 ...
 * atomic<> integers, to avoid locking.  Using a atomic<> is safe
...
void run_counters_test(arg_t &args) {
	...
	struct padded_counter {
		...
		char padding[128 - sizeof(atomic<uint64_t>)];
	};
