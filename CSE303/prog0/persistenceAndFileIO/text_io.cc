 ...
 */
void read_lines_file(FILE *file, function<void(const char *, size_t)> cb) {
 ...
 *           number of valid bytes reachable from that pointer.
 */
void read_lines_fd(int fd, function<void(const char *, size_t)> cb) {
	// read data into this space on the stack
	char buffer[12];
...
int main(int argc, char **argv) {
	...
	if (args.out_file != "") {
		...
		if (args.append)
			out_fd = open(args.out_file.c_str(), O_WRONLY | O_CREAT | O_APPEND, S_IRWXU);
		else
	...
	function<void(const char *, size_t)> print_stream = [&](const char *buf, size_t) { write_file(out_stream, buf); };
	function<void(const char *, size_t)> print_fd = [&](const char *buf, size_t num) { write_fd(out_fd, buf, num); };
