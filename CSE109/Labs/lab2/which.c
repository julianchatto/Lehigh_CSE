...
int main(int argc, char** argv) { // also known as: char* argv[]
	...
	for (int i = itemp; i < argc; i++) { // looping through arguments
		bool found = false;
		int tmpCount = 0;
...
/* 
 * Q0: The “#ifndef… #endif” is called an include guard. Search online
 *     to learn the reason for using an include guard.
 * A0: include guards are used to prevent the contents of a file from being included multiple times
 *	
 * Q1: Search through the /usr/include/errno.h looking for the highest
 *     errno value. You may have to look in other errno.h files that 
 *     are in “#include…” statements in the .h files.
 * A1: The highest errno value is 133
 */ 
